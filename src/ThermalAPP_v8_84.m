function ThermalPro_v8_84
    % ThermalPro —— EBWAM 过程红外热成像监控与分析工具
    % 用于处理电子束线材增材制造过程中拍摄的红外热成像序列，
    % 实现熔池 / HAZ 面积定量、非线性温度标定、层间暂停识别等功能。
    %
    % 依赖: Image Processing Toolbox, MATLAB R2020b+
    % 用法: 在命令窗口输入 ThermalPro_v8_84 启动 GUI
    %
    % Copyright (C) 2025  Flanker
    % 本程序遵循 GPL v3 协议
    ...
    S.vapor.refTemp = 1500;     % 全局参考温度
    S.vapor.refSchedule = [];   % [起始帧, 结束帧, 参考温度]
    S.vapor.anchors = [];       % 锚点列表
    S.vapor.isPicking = false;
    S.vapor.gammaSchedule = [];
    S.vapor.bgRect = []; S.vapor.bgCurve = []; S.vapor.bgRefVal = 0; % 背景校正(去雾)

    S.hAutoCircle = [];

    % 标定模式 (Single = 单点全局, Multi = 多层热图)
    S.calibMode = 'Single';
    S.heatCurve = [];
    ...
    % 动态 ROI 锚定 (局部特征追踪, 熔池免疫)
    S.dynamicAnchor = struct('isActive', false, 'refFrame', 1, 'refRect', [], 'localShifts', [], 'isCalculated', false);
    S.analysisBaseCoords = {}; % 动态渲染用的原始坐标
    
    %% 面积分析参数 (Area analysis)
    S.area = struct();
    S.area.enabled = false;               % 是否显示轮廓叠加
    S.area.T_melt = 1400;                 % 熔池判定温度 (°C) — 仅非自适应模式用
    S.area.T_haz = 980;                   % HAZ 判定温度 (°C)
    S.area.mpHistory = [];                % 各帧熔池面积历史
    S.area.hazHistory = [];               % 各帧 HAZ 面积历史
    S.area.hMPHistory = [];               % 熔池 BBox 高度
    S.area.lMPHistory = [];               % 熔池 BBox 宽度
    S.area.hHAZHistory = [];              % HAZ BBox 高度
    S.area.lHAZHistory = [];              % HAZ BBox 宽度
    S.area.morphKernel = 3;               % 形态学开运算核大小
    S.area.plotMode = 1;                  % 曲线图显示模式
    S.area.proximityThresh = 2.0;         % MP-HAZ 邻近距离阈值 (px)
    S.area.minRatio = 20.0;               % MP:HAZ 最小面积比
    S.area.forcedFrames = [];             % 强制纳入的帧号
    S.area.blacklistedFrames = [];        % 强制排除的帧号
    S.area.isCalibrated = false;          % 是否完成空间标定
    S.area.pxPerMm = 1.0;                 % 像素-毫米换算比
    S.area.adaptiveEnabled = false;       % 启用梯度边界自适应阈值
    S.area.boundaryTemps = [];            % 逐帧边界温度缓存
    S.area.hazRatio = 0.55;               % HAZ阈值 = 熔池边界 × hazRatio
    S.area.minBoundRatio = 1.00;          % 边界温度下限 = refTemp × 此值
    S.area.filterFlashes = false;         % 启用束斑追踪层间过滤
    S.area.interpassMode = 'exclude';     % 过滤模式: exclude/only/all
    S.area.cxHistory = [];                % 逐帧束斑 x 坐标
    S.area.isInterpass = [];              % 逐帧层间暂停标记
    S.area.maxMeltArea = 700;             % 熔池面积上限 (px, 0=关闭)
    S.area.maxHazArea = 1500;             % HAZ 面积上限 (px, 0=关闭)
    S.area.rejectReason = {};            % 逐帧剔除原因文本

    % NDT 层厚真值映射
    S.ndt = struct();
    S.ndt.gtTable = cell(0, 4); % {层号, 真实深度, 原始Z, 处理后Z}
    S.ndt.baselinePos = [];
    S.ndt.layerLinesRaw = {};
    ...
    %% --- GUI 构建 ---
    fig = figure('Name','ThermalPro v8.84 | EBWAM Interactive Lab','NumberTitle','off',...
    ...
    function onLoadFile(~,~)
        ...
        try
            ...
            if isProject
                ...
                %Migrate old HTW field names to HAZ (v8.83 rename)
    ...
    function computeAdaptiveThresholds()
        %Pre-scan all frames with gradient boundary detection.
        ...
        %Beam-position-based interpass classifier.
    ...
    function onCalcArea(~,~)
        ...
        %Compute adaptive boundary temperatures if enabled
        if S.area.adaptiveEnabled && isempty(S.area.boundaryTemps)
    ...
    function bTemp = detectMeltPoolBoundaryTemp(img, refTemp, minRadials)
        % 基于径向梯度分析的熔池边界检测 —— 核心标定算法
        % 原理：固-液界面处热梯度最大（相变潜热导致温度骤降）。
        % 从束斑中心向外做 36 向径向扫描，取每条剖面上 |dT/dr| 峰值
        % 位置的温度作为边界温度。经 6 重门控过滤假阳性。
        % 返回热像仪在边界处的读数，若检测失败则返回 NaN。
        % 输入: img-单帧图像, refTemp-材料熔点对应的相机读数, minRadials-最少有效方向数
        if nargin < 3, minRadials = 3; end
        [H, W] = size(img);
        [maxVal, maxIdx] = max(img(:));
        % 预筛选：帧内最高温连熔点的一半都不到 → 不可能有熔池
        if isempty(maxVal) || maxVal < (refTemp * 0.5)
            bTemp = NaN; return;
        end
        % 最高温远低于预期相变温度 → 不可能存在液相
        ...
        if isempty(boundTemps)
            ...
            %False-positive rejection pipeline (5 guards).
            %
