function ThermalPro_v8_83
    % ThermalPro v8.83 - EBWAM Analysis (Adaptive Threshold Area Detection)
    %
    % [FIX LOG v8.83 (Latest)]
    % 1. FEATURE (Adaptive Area Threshold): Area analysis now supports gradient-boundary-driven
    %    adaptive thresholds. Instead of fixed T_melt/T_haz for all frames, each frame's
    %    melt pool boundary temperature is detected via radial gradient analysis and used as
    %    the frame-specific T_melt. T_haz is derived as T_boundary * hazRatio (default 0.55).
    %    This automatically adapts to beam power variations without user intervention.
    % 2. FEATURE (Adaptive Toggle): New "Adaptive (Gradient)" checkbox in Area Analysis tab.
    %    When enabled, computeAdaptiveThresholds pre-scans all frames before area calculation.
    % 3. FEATURE (Boundary Temp Cache): S.area.boundaryTemps stores per-frame boundary
    %    temperatures, reused across area re-calculations until invalidated.
    % 4. REFACTOR (HTW -> HAZ): Renamed all "HTW" identifiers to "HAZ" throughout the codebase.
    % 5. BUG FIX (False Melt Pool Rejection): detectMeltPoolBoundaryTemp now has a 5-guard
    %    pipeline: (1) thermal contrast > 50 C, (2) boundary distance < W/4 from centre,
    %    (3) gradient > 15 C/px, (4) boundary/peak temperature ratio > 0.70,
    %    (5) minimum valid radial directions. Catches beam-off, preheat, and cool-down frames.
    % 6. BUG FIX (Project Load Migration): Added explicit field-name migration from old
    %    HTW names (T_htw, htwRatio, etc.) to new HAZ names when loading v8.82 projects.
    %
    % [FIX LOG v8.82]
    ...
    % 3. PHYSICS FIX (HAZ Threshold): Changed default T_haz from 600 C to 980 C (TC4 beta-transus ~980 C).
    ...
    S.area.hHAZHistory = [];         % [NEW] Size H for HAZ
    S.area.lHAZHistory = [];         % [NEW] Size L for HAZ
    ...
    S.area.pxPerMm = 1.0;
    S.area.adaptiveEnabled = false;      % [v8.83] gradient-boundary adaptive threshold
    S.area.boundaryTemps = [];           % [v8.83] per-frame boundary temp cache
    S.area.hazRatio = 0.55;             % [v8.83] T_haz = T_boundary * hazRatio
    S.area.minBoundRatio = 1.00;        % [v8.83] floor for boundary T = refTemp * ratio
    S.area.filterFlashes = false;        % [v8.83] enable interpass filtering
    S.area.interpassMode = 'exclude';     % [v8.83] 'exclude'|'only'|'all'
    S.area.cxHistory = [];               % [v8.83] beam x-position per frame
    S.area.isInterpass = [];             % [v8.83] per-frame interpass flag
    S.area.maxMeltArea = 700;            % [v8.83] max MP px, >threshold = interpass
    S.area.maxHazArea = 1500;            % [v8.83] max HAZ px (0=off)
    S.area.rejectReason = {};            % [v8.83] per-frame ban reason string
    ...
    uicontrol('Parent',sTabAuto,'Style','text','String','HAZ T:','Units','normalized',...
    ...
    S.hUI.popAreaDisplay = uicontrol('Parent',sTabAuto,'Style','popupmenu',...
        'String',{'Show All', 'MP Only', 'HAZ Only'},...
    ...
    uicontrol('Parent',sTabAuto,'Style','pushbutton','String','▶ Calculate Full Frame Curve',...
        'Units','normalized','Position',[.05 .25 .90 .20],'BackgroundColor',[.8 .85 .95],...
        'FontWeight', 'bold', 'FontSize', 10, 'Callback',@onCalcArea);

    % Sub-Tab 2: Adaptive (gradient boundary + flash filter)
    sTabAdaptive = uitab('Parent', subTabGroup, 'Title', 'Adaptive');

    S.hUI.chkAdaptive = uicontrol('Parent',sTabAdaptive,'Style','checkbox',...
        'String','Adaptive (Gradient Boundary)',...
        'Units','normalized','Position',[.02 .82 .55 .12],'Value',0,...
        'BackgroundColor',[1 1 1],'Callback',@onAreaConfigChange,...
        'TooltipString','Use per-frame gradient boundary detection. Adapts to power changes.');

    uicontrol('Parent',sTabAdaptive,'Style','text','String','HAZ Ratio:','Units','normalized',...
        'Position',[.02 .65 .15 .10],'HorizontalAlignment','right');
    S.hUI.efAdaptiveRatio = uicontrol('Parent',sTabAdaptive,'Style','edit','String','0.55',...
        'Units','normalized','Position',[.18 .65 .12 .10],'Callback',@onAreaConfigChange,...
        'TooltipString','T_haz = T_boundary * ratio. Default 0.55.');
    uicontrol('Parent',sTabAdaptive,'Style','text','String','Min Boundary:','Units','normalized',...
        'Position',[.35 .65 .18 .10],'HorizontalAlignment','right');
    S.hUI.efMinBoundRatio = uicontrol('Parent',sTabAdaptive,'Style','edit','String','1.00',...
        'Units','normalized','Position',[.54 .65 .12 .10],'Callback',@onAreaConfigChange,...
        'TooltipString','Tm_i >= refTemp * ratio. Default 1.00 = refTemp absolute floor.');

    S.hUI.chkFilterFlashes = uicontrol('Parent',sTabAdaptive,'Style','checkbox',...
        'String','Filter Interpass (beam tracking)',...
        'Units','normalized','Position',[.02 .45 .55 .12],'Value',0,...
        'BackgroundColor',[1 1 1],'Callback',@onAreaConfigChange,...
        'TooltipString','Track beam spot motion to separate printing from interpass.');

    uicontrol('Parent',sTabAdaptive,'Style','text','String','Mode:','Units','normalized',...
        'Position',[.02 .30 .10 .10],'HorizontalAlignment','right');
    S.hUI.popInterpassMode = uicontrol('Parent',sTabAdaptive,'Style','popupmenu',...
        'String',{'Exclude Interpass','Interpass Only','All Frames'},...
        'Units','normalized','Position',[.13 .30 .40 .12],'Callback',@onAreaConfigChange,...
        'TooltipString','Exclude: keep only printing. Interpass Only: keep only pauses. All: keep everything.');

    uicontrol('Parent',sTabAdaptive,'Style','text','String','Max MP Area (px):','Units','normalized',...
        'Position',[.02 .15 .20 .10],'HorizontalAlignment','right');
    S.hUI.efMaxMeltArea = uicontrol('Parent',sTabAdaptive,'Style','edit','String','700',...
        'Units','normalized','Position',[.23 .15 .10 .10],'Callback',@onAreaConfigChange,...
        'TooltipString','Frames with MP > this are forced to interpass. 0 = off.');
    uicontrol('Parent',sTabAdaptive,'Style','text','String','Max HAZ Area (px):','Units','normalized',...
        'Position',[.38 .15 .20 .10],'HorizontalAlignment','right');
    S.hUI.efMaxHazArea = uicontrol('Parent',sTabAdaptive,'Style','edit','String','1500',...
        'Units','normalized','Position',[.59 .15 .10 .10],'Callback',@onAreaConfigChange,...
        'TooltipString','Frames with HAZ > this are forced to interpass. 0 = off.');

    % Sub-Tab 3: Manual Fix
    ...
    function onLoadFile(~,~)
        ...
        try
            ...
            if isProject
                ...
                % [v8.83] Migrate old HTW field names to HAZ (v8.83 rename)
                if isfield(S.area, 'T_htw') && ~isfield(S.area, 'T_haz'), S.area.T_haz = S.area.T_htw; end
                if isfield(S.area, 'htwHistory') && ~isfield(S.area, 'hazHistory'), S.area.hazHistory = S.area.htwHistory; end
                if isfield(S.area, 'hHTWHistory') && ~isfield(S.area, 'hHAZHistory'), S.area.hHAZHistory = S.area.hHTWHistory; end
                if isfield(S.area, 'lHTWHistory') && ~isfield(S.area, 'lHAZHistory'), S.area.lHAZHistory = S.area.lHTWHistory; end
                if isfield(S.area, 'htwRatio') && ~isfield(S.area, 'hazRatio'), S.area.hazRatio = S.area.htwRatio; end
                % [v8.78 Forward Compatibility]: Pre-allocate missing fields for old projects
                if ~isfield(S.area, 'hMPHistory'), S.area.hMPHistory = []; end
                if ~isfield(S.area, 'lMPHistory'), S.area.lMPHistory = []; end
                if ~isfield(S.area, 'hHAZHistory'), S.area.hHAZHistory = []; end
                if ~isfield(S.area, 'lHAZHistory'), S.area.lHAZHistory = []; end
                ...
                if ~isfield(S.area, 'T_haz') || isnan(S.area.T_haz) || S.area.T_haz <= 0 || S.area.T_haz >= S.area.T_melt, S.area.T_haz = 980; end
                if ~isfield(S.area, 'adaptiveEnabled'), S.area.adaptiveEnabled = false; end
                if ~isfield(S.area, 'boundaryTemps'), S.area.boundaryTemps = []; end
                if ~isfield(S.area, 'hazRatio') || isnan(S.area.hazRatio) || S.area.hazRatio <= 0, S.area.hazRatio = 0.55; end
                if ~isfield(S.area, 'minBoundRatio') || isnan(S.area.minBoundRatio) || S.area.minBoundRatio <= 0, S.area.minBoundRatio = 1.00; end
                if ~isfield(S.area, 'filterFlashes'), S.area.filterFlashes = false; end
                if ~isfield(S.area, 'interpassMode'), S.area.interpassMode = 'exclude'; end
                if ~isfield(S.area, 'cxHistory'), S.area.cxHistory = []; end
                if ~isfield(S.area, 'isInterpass'), S.area.isInterpass = []; end
                if ~isfield(S.area, 'maxMeltArea') || isnan(S.area.maxMeltArea) || S.area.maxMeltArea < 0, S.area.maxMeltArea = 700; end
                if ~isfield(S.area, 'maxHazArea') || isnan(S.area.maxHazArea) || S.area.maxHazArea < 0, S.area.maxHazArea = 1500; end
                if ~isfield(S.area, 'rejectReason'), S.area.rejectReason = {}; end
                ...
                S.area.mpHistory = []; S.area.hazHistory = [];
                S.area.hMPHistory = []; S.area.lMPHistory = [];
                S.area.hHAZHistory = []; S.area.lHAZHistory = [];
                S.area.forcedFrames = []; S.area.blacklistedFrames = []; 
                S.area.isCalibrated = false; S.area.pxPerMm = 1.0;
                S.area.adaptiveEnabled = false; S.area.boundaryTemps = []; S.area.hazRatio = 0.55; S.area.minBoundRatio = 1.00; S.area.filterFlashes = false; S.area.interpassMode = 'exclude'; S.area.cxHistory = []; S.area.isInterpass = []; S.area.maxMeltArea = 700; S.area.maxHazArea = 1500;

                ylabel(axArea, 'Area (px^2)');
    ...
    function restoreUiState()
        ...
        set(S.hUI.efAreaHAZ, 'String', num2str(S.area.T_haz));
        ...
        set(S.hUI.chkAdaptive, 'Value', S.area.adaptiveEnabled);
        set(S.hUI.efAdaptiveRatio, 'String', num2str(S.area.hazRatio));
        set(S.hUI.efMinBoundRatio, 'String', num2str(S.area.minBoundRatio));
        set(S.hUI.chkFilterFlashes, 'Value', S.area.filterFlashes);
        modeVal = 1;
        if strcmp(S.area.interpassMode, 'only'), modeVal = 2;
        elseif strcmp(S.area.interpassMode, 'all'), modeVal = 3; end
        set(S.hUI.popInterpassMode, 'Value', modeVal);
        set(S.hUI.efMaxMeltArea, 'String', num2str(S.area.maxMeltArea));
        set(S.hUI.efMaxHazArea, 'String', num2str(S.area.maxHazArea));
        onVaporConfigChange();
    end
    ...
    function updateFrame(idx)
        ...
        if S.area.enabled
            skipOverlay = false;
            if S.area.adaptiveEnabled && isempty(S.area.boundaryTemps)
                Tm_i = S.area.T_melt; Th_i = S.area.T_haz;  % preview before Calculate
            else
                [Tm_i, Th_i] = getMeltThresholds(imgData, idx);
                if isnan(Tm_i), skipOverlay = true; end
            end
            if ~skipOverlay
            hold(axMain, 'on'); bwMP = imgData >= Tm_i; bwHAZ = (imgData >= Th_i) & (imgData < Tm_i); se = strel('disk', S.area.morphKernel); bwHAZ = imopen(bwHAZ, se);
            CC_HAZ = bwconncomp(bwHAZ);
            if CC_HAZ.NumObjects > 0, numPixels = cellfun(@numel, CC_HAZ.PixelIdxList); [~, maxIdx] = max(numPixels); bwHAZ = false(size(bwHAZ)); bwHAZ(CC_HAZ.PixelIdxList{maxIdx}) = true; end
            ...
            elseif isForced
                ...
                if CC_MP.NumObjects > 0
                    ...
                    else, isValidFrame = true; diagStatus = 'VALID (No HAZ)'; end
            ...
            monitorStr = sprintf('[%s] Dist: %s | Ratio: %s | MP: %.1f%s | HAZ: %.1f%s', diagStatus, strDist, strRatio, areaMP*scaleFactor, unitStr, areaHAZ*scaleFactor, unitStr);
            ...
            hold(axMain, 'off');
                ...
                reason = 'no boundary data';
                if ~isempty(S.area.rejectReason) && idx <= length(S.area.rejectReason) ...
                        && ~isempty(S.area.rejectReason{idx})
                    reason = S.area.rejectReason{idx};
                end
                % Show actual MP/HAZ even when banned
                mpShow = 0; hazShow = 0;
                if S.area.adaptiveEnabled && ~isempty(S.area.boundaryTemps) ...
                        && idx <= length(S.area.boundaryTemps) && ~isnan(S.area.boundaryTemps(idx))
                    Tm_tmp = max(S.area.boundaryTemps(idx), S.vapor.refTemp * S.area.minBoundRatio);
                    Th_tmp = Tm_tmp * S.area.hazRatio;
                    mpShow = sum(imgData(:) >= Tm_tmp);
                    hazShow = sum(imgData(:) >= Th_tmp & imgData(:) < Tm_tmp);
                end
                set(S.hUI.lblAreaDiag, 'String', sprintf('BANNED: %s | MP:%.0f HAZ:%.0f px', reason, mpShow, hazShow), 'BackgroundColor', [1 .85 .85]);
            end  % ~skipOverlay
        end
    ...
    function ensureHistoryExists()
        if isempty(S.area.mpHistory) || length(S.area.mpHistory) ~= S.N
            S.area.mpHistory = zeros(S.N, 1); S.area.hazHistory = zeros(S.N, 1);
            S.area.hMPHistory = zeros(S.N, 1); S.area.lMPHistory = zeros(S.N, 1);
            S.area.hHAZHistory = zeros(S.N, 1); S.area.lHAZHistory = zeros(S.N, 1);
        end
    end

    % --- Shared threshold helper (v8.83): single source of truth for ---
    %     adaptive / fixed / clamp / safety-net logic.  All three callers
    %     (updateFrame, performSingleFrameCalc, onCalcArea) route through
    %     this function so that any future change applies everywhere.
    function [Tm_i, Th_i, isAdaptive] = getMeltThresholds(img, idx)
        isAdaptive = S.area.adaptiveEnabled;
        if isAdaptive
            if isempty(S.area.boundaryTemps) || idx > length(S.area.boundaryTemps) || isnan(S.area.boundaryTemps(idx))
                Tm_i = NaN; Th_i = NaN; setReason('no boundary in cache'); return;
            end
            % Beam-tracker interpass filter
            if S.area.filterFlashes && ~isempty(S.area.isInterpass) && idx <= length(S.area.isInterpass)
                if strcmp(S.area.interpassMode, 'exclude') && S.area.isInterpass(idx)
                    Tm_i = NaN; Th_i = NaN; setReason('interpass (beam tracker)'); return;
                elseif strcmp(S.area.interpassMode, 'only') && ~S.area.isInterpass(idx)
                    Tm_i = NaN; Th_i = NaN; setReason('printing (interpass-only mode)'); return;
                end
            end
            Tm_i = max(S.area.boundaryTemps(idx), S.vapor.refTemp * S.area.minBoundRatio);
            Th_i = Tm_i * S.area.hazRatio;
            % Max-area hard ceiling
            if S.area.maxMeltArea > 0 && sum(img(:) >= Tm_i) > S.area.maxMeltArea
                Tm_i = NaN; Th_i = NaN; setReason(sprintf('MP > %d px', S.area.maxMeltArea)); return;
            end
            if S.area.maxHazArea > 0
                hazCount = sum(img(:) >= Th_i & img(:) < Tm_i);
                if hazCount > S.area.maxHazArea
                    Tm_i = NaN; Th_i = NaN; setReason(sprintf('HAZ > %d px', S.area.maxHazArea)); return;
                end
            end
            % Safety net: adaptive threshold produced < 5 px but the frame
            % IS hot enough to possibly contain a melt pool.  The three
            % conditions together ensure we only rescue real melt pools
            % suffering from emissivity drift — not interpass flashes
            % (which never reach refTemp) or cold frames.
            if sum(img(:) >= Tm_i) < 5 ...
                    && max(img(:)) >= S.vapor.refTemp ...
                    && S.area.boundaryTemps(idx) > S.vapor.refTemp * 0.90
                Tm_i = max(img(:)) * 0.88; Th_i = Tm_i * S.area.hazRatio;
            end
        else
            Tm_i = S.area.T_melt; Th_i = S.area.T_haz;
        end

        function setReason(msg)
            if isempty(S.area.rejectReason), return; end
            if idx <= length(S.area.rejectReason)
                S.area.rejectReason{idx} = msg;
            end
        end
    end

    function [valMP, valHAZ, hMP, lMP, hHAZ, lHAZ] = performSingleFrameCalc(idx)
        img = getProcessedFrame(idx);
        [Tm_i, Th_i, ~] = getMeltThresholds(img, idx);
        if isnan(Tm_i), valMP = 0; valHAZ = 0; hMP = 0; lMP = 0; hHAZ = 0; lHAZ = 0; return; end
        bwMP = img >= Tm_i; bwHAZ = (img >= Th_i) & (img < Tm_i);
    ...
    function onForceValid(~,~)
        ...
        if ~ismember(S.frameIdx, S.area.forcedFrames)
            ...
            S.area.mpHistory(S.frameIdx) = valMP; S.area.hazHistory(S.frameIdx) = valHAZ; 
            S.area.hMPHistory(S.frameIdx) = hMP; S.area.lMPHistory(S.frameIdx) = lMP;
            S.area.hHAZHistory(S.frameIdx) = hHAZ; S.area.lHAZHistory(S.frameIdx) = lHAZ;
    ...
    function onForceRemove(~,~)
        ...
        if ~ismember(S.frameIdx, S.area.blacklistedFrames)
            ...
            S.area.mpHistory(S.frameIdx) = 0; S.area.hazHistory(S.frameIdx) = 0; 
            S.area.hMPHistory(S.frameIdx) = 0; S.area.lMPHistory(S.frameIdx) = 0;
            S.area.hHAZHistory(S.frameIdx) = 0; S.area.lHAZHistory(S.frameIdx) = 0;
            plotAreaCurves(); updateFrame(S.frameIdx); 
        end
    end

    function computeAdaptiveThresholds()
        % [v8.83] Pre-scan all frames with gradient boundary detection.
        % Fills S.area.boundaryTemps with per-frame boundary temperatures.
        % Frames without a detectable melt pool get NaN (filled by interpolation).
        hWaitB = waitbar(0, 'Computing adaptive boundary thresholds...', ...
            'CreateCancelBtn', 'setappdata(gcbf,''canceling'',1)');
        setappdata(hWaitB, 'canceling', 0);
        S.area.boundaryTemps = nan(S.N, 1);
        S.area.rejectReason = cell(S.N, 1);  % reset per-frame ban reasons
        cx = zeros(S.N, 1);  % beam x-position for interpass detection
        hp = zeros(S.N, 1);  % hot-pixel count (>= refTemp) for area-change detection
        isCanceled = false;
        try
            for i = 1:S.N
                if ~isvalid(hWaitB) || getappdata(hWaitB, 'canceling')
                    isCanceled = true; break;
                end
                raw = getProcessedFrame(i);
                [~, maxIdx] = max(raw(:));
                [~, cx(i)] = ind2sub(size(raw), maxIdx);
                hp(i) = sum(raw(:) >= S.vapor.refTemp);
                S.area.boundaryTemps(i) = detectMeltPoolBoundaryTemp(raw, S.vapor.refTemp, 12);
                if mod(i, 200) == 0 && isvalid(hWaitB)
                    nFound = sum(~isnan(S.area.boundaryTemps(1:i)));
                    waitbar(i/S.N, hWaitB, sprintf('Frame %d/%d | %d boundaries', i, S.N, nFound));
                end
            end
        catch ME
            if isvalid(hWaitB), delete(hWaitB); end
            errordlg(['Adaptive threshold computation failed: ' ME.message]); return;
        end
        if isvalid(hWaitB), delete(hWaitB); end
        if isCanceled
            S.area.boundaryTemps = []; msgbox('Adaptive threshold cancelled.', 'Cancelled'); return;
        end
        nFound = sum(~isnan(S.area.boundaryTemps));
        if nFound < 2
            warndlg(sprintf('Only %d frames with detectable boundary.\nFalling back to fixed thresholds.', nFound));
            S.area.boundaryTemps = []; return;
        end

        % [v8.83] Beam-position-based interpass classifier.
        %   Printing: beam moves L↔R across the part.
        %   Interpass: beam stationary at one end, hot zone expands.
        %   Tracks the x-coordinate of the hottest pixel to detect motion.
        S.area.cxHistory = cx;
        if S.area.filterFlashes
            % --- Beam-position displacement classifier ---
            %   Printing: beam has moved >15 px in the last 50 frames.
            %   Interpass: beam displacement <15 px in 50 frames.
            %   Uses net displacement (not instantaneous velocity) to
            %   correctly handle slow scans and avoid state-machine lock-in.
            cxSmooth = movmedian(cx, 11);
            WIN = 50;
            % Backward displacement: how far the beam moved in past 50 fr
            netDisp = zeros(S.N, 1);
            for i = WIN:S.N
                netDisp(i) = abs(cxSmooth(i) - cxSmooth(i - WIN + 1));
            end
            % Forward displacement: how far the beam WILL move in next 50 fr
            netDispFwd = zeros(S.N, 1);
            for i = 1:S.N-WIN+1
                netDispFwd(i) = abs(cxSmooth(i+WIN-1) - cxSmooth(i));
            end
            % Stationary only if BOTH backward and forward displacement < 15
            isStationary = netDisp < 15 & netDispFwd < 15;

            % Stationary segments ≥ 30 fr (6 s @ 5 Hz) = interpass
            isInterpass = false(S.N, 1);
            st = find(diff([0; isStationary]) == 1);
            en = find(diff([isStationary; 0]) == -1);
            for k = 1:length(st)
                if en(k) - st(k) + 1 >= 30
                    isInterpass(st(k):en(k)) = true;
                end
            end

            S.area.isInterpass = isInterpass;
            fprintf('Beam tracker: %d printing, %d interpass frames (mode=%s)\n', ...
                sum(~isInterpass), sum(isInterpass), S.area.interpassMode);
        end

        firstValid = find(~isnan(S.area.boundaryTemps), 1, 'first');
        lastValid  = find(~isnan(S.area.boundaryTemps), 1, 'last');
        S.area.boundaryTemps = fillmissing(S.area.boundaryTemps, 'linear');
        S.area.boundaryTemps = movmedian(S.area.boundaryTemps, 11, 'omitnan');
        if ~isempty(firstValid), S.area.boundaryTemps(1:firstValid-1) = NaN; end
        if ~isempty(lastValid),  S.area.boundaryTemps(lastValid+1:end) = NaN; end

        msgbox(sprintf('Adaptive thresholds ready.\n%d/%d frames with melt pool detected.\nFrames %d-%d: gradient boundary.', nFound, S.N, firstValid, lastValid));
    end

    function onCalcArea(~,~)
        if ~S.isLoaded, errordlg('Please load data first.'); return; end
        T_m = readPositiveScalar(S.hUI.efAreaMP, S.area.T_melt, 'Melt Pool Threshold', true);
        T_h = readPositiveScalar(S.hUI.efAreaHAZ, S.area.T_haz, 'HAZ Threshold', true);
        if T_h >= T_m && ~S.area.adaptiveEnabled
            errordlg('HAZ threshold must be lower than Melt Pool threshold.'); return;
        end
        S.area.T_melt = T_m; S.area.T_haz = T_h;
        S.area.morphKernel = max(1, round(S.area.morphKernel));
        set(S.hUI.efMorph, 'String', num2str(S.area.morphKernel));

        % [v8.83] Compute adaptive boundary temperatures if enabled
        if S.area.adaptiveEnabled && isempty(S.area.boundaryTemps)
            computeAdaptiveThresholds();
        end

        % [FIX v8.80 Bug2] Add Cancel button
        hWait = waitbar(0, 'Processing Area & Dimensional Statistics...', ...
            'CreateCancelBtn', 'setappdata(gcbf,''canceling'',1)');
        setappdata(hWait, 'canceling', 0);
        mp_hist = zeros(S.N, 1); haz_hist = zeros(S.N, 1);
        h_mp = zeros(S.N, 1); l_mp = zeros(S.N, 1);
        h_haz = zeros(S.N, 1); l_haz = zeros(S.N, 1);
        ...
        try
            ...
            for i = 1:S.N
                ...
                if ismember(i, S.area.blacklistedFrames)
                    mp_hist(i) = 0; haz_hist(i) = 0; continue;
                end
                img = getProcessedFrame(i);
                [Tm_i, Th_i] = getMeltThresholds(img, i);
                if isnan(Tm_i), mp_hist(i) = 0; haz_hist(i) = 0; continue; end
                bwMP = img >= Tm_i; bwHAZ = (img >= Th_i) & (img < Tm_i); bwHAZ = imopen(bwHAZ, se);
                CC_HAZ = bwconncomp(bwHAZ);
                if CC_HAZ.NumObjects > 0, numPixels = cellfun(@numel, CC_HAZ.PixelIdxList); [~, maxIdx] = max(numPixels); bwHAZ = false(size(bwHAZ)); bwHAZ(CC_HAZ.PixelIdxList{maxIdx}) = true; end;
                if ismember(i, S.area.forcedFrames)
                    mp_hist(i) = sum(bwMP(:)); haz_hist(i) = sum(bwHAZ(:));
                    [l_mp(i), h_mp(i), ~, ~] = getMorphDims(bwMP);
                    [l_haz(i), h_haz(i), ~, ~] = getMorphDims(bwHAZ);
                    continue;
                end
                mpCount = 0; hazCount = 0;
                hMPVal = 0; lMPVal = 0; hHAZVal = 0; lHAZVal = 0;
                CC_MP = bwconncomp(bwMP);
                if CC_MP.NumObjects > 0
                    if any(bwHAZ(:)) && CC_MP.NumObjects > 1
                        D_HAZ = bwdist(bwHAZ); minDists = zeros(1, CC_MP.NumObjects); for k = 1:CC_MP.NumObjects, minDists(k) = min(D_HAZ(CC_MP.PixelIdxList{k})); end;
                    ...
                    areaMP = sum(bwMP(:)); areaHAZ = sum(bwHAZ(:));
                    if areaHAZ > 0
                        D_MP = bwdist(bwMP); minDist = min(D_MP(bwHAZ)); ratioOk = areaMP >= (areaHAZ / S.area.minRatio);
                        if minDist <= S.area.proximityThresh && ratioOk
                            mpCount = areaMP; hazCount = areaHAZ;
                            [lMPVal, hMPVal, ~, ~] = getMorphDims(bwMP);
                            [lHAZVal, hHAZVal, ~, ~] = getMorphDims(bwHAZ);
                        end
                    else
                        mpCount = areaMP;
                        [lMPVal, hMPVal, ~, ~] = getMorphDims(bwMP);
                ...
                mp_hist(i) = mpCount; haz_hist(i) = hazCount;
                h_mp(i) = hMPVal; l_mp(i) = lMPVal;
                h_haz(i) = hHAZVal; l_haz(i) = lHAZVal;
            ...
            if isCanceled
                ...
                S.area.mpHistory = mp_hist; S.area.hazHistory = haz_hist;
                S.area.hMPHistory = h_mp; S.area.lMPHistory = l_mp;
                S.area.hHAZHistory = h_haz; S.area.lHAZHistory = l_haz;
                plotAreaCurves();
            end
    ...
    function plotAreaCurves()
        ...
        % Data scaling
        mpA = S.area.mpHistory * scaleArea; hazA = S.area.hazHistory * scaleArea;
        hMP = S.area.hMPHistory * scaleLine; lMP = S.area.lMPHistory * scaleLine;
        hHAZ = S.area.hHAZHistory * scaleLine; lHAZ = S.area.lHAZHistory * scaleLine;
        ...
        if mode == 1 || mode == 3
             plot(axArea, xData, hazA, 'Color', [0.6 0 0.8], 'LineWidth', 1.5, 'DisplayName', 'HAZ Area');
             plot(axSizeH, xData, hHAZ, 'Color', [0.6 0 0.8], 'LineWidth', 1.5, 'DisplayName', 'HAZ Size H');
             plot(axSizeL, xData, lHAZ, 'Color', [0.6 0 0.8], 'LineWidth', 1.5, 'DisplayName', 'HAZ Size L');
        end
    ...
    function onExportAreaCSV(~,~)
        ...
        if f
            Frames = (1:S.N)';
            Time = Frames / S.fps;
            T = table(Frames, Time, S.area.mpHistory*scaleArea, S.area.hazHistory*scaleArea, ...
                S.area.hMPHistory*scaleLine, S.area.lMPHistory*scaleLine, S.area.hHAZHistory*scaleLine, S.area.lHAZHistory*scaleLine, ...
                'VariableNames', {'Frame', 'Time_s', ['MeltPoolArea' uArea], ['HAZArea' uArea], ...
                ['MP_SizeH' uLine], ['MP_SizeL' uLine], ['HAZ_SizeH' uLine], ['HAZ_SizeL' uLine]});
    ...
    function onAreaConfigChange(~,~)
        ...
        newTh = readPositiveScalar(S.hUI.efAreaHAZ, S.area.T_haz, 'HAZ Threshold', true);
        if newTh >= newTm && ~get(S.hUI.chkAdaptive, 'Value')
            warndlg('HAZ threshold must be lower than Melt Pool threshold. Reverted to previous values.', 'Invalid Area Thresholds');
            set(S.hUI.efAreaMP, 'String', num2str(S.area.T_melt));
            set(S.hUI.efAreaHAZ, 'String', num2str(S.area.T_haz));
        else
            S.area.T_melt = newTm;
        ...
        S.area.adaptiveEnabled = get(S.hUI.chkAdaptive, 'Value');
        S.area.minBoundRatio = readPositiveScalar(S.hUI.efMinBoundRatio, S.area.minBoundRatio, 'Min Boundary', false);
        if S.area.minBoundRatio > 1.0, S.area.minBoundRatio = 1.00; end
        if S.area.minBoundRatio < 0.5, S.area.minBoundRatio = 0.50; end
        set(S.hUI.efMinBoundRatio, 'String', num2str(S.area.minBoundRatio));
        S.area.filterFlashes = get(S.hUI.chkFilterFlashes, 'Value');
        S.area.maxMeltArea = max(0, round(readPositiveScalar(S.hUI.efMaxMeltArea, S.area.maxMeltArea, 'Max MP Area', false)));
        set(S.hUI.efMaxMeltArea, 'String', num2str(S.area.maxMeltArea));
        S.area.maxHazArea = max(0, round(readPositiveScalar(S.hUI.efMaxHazArea, S.area.maxHazArea, 'Max HAZ Area', false)));
        set(S.hUI.efMaxHazArea, 'String', num2str(S.area.maxHazArea));
        modeVal = get(S.hUI.popInterpassMode, 'Value');
        switch modeVal
            case 1, S.area.interpassMode = 'exclude';
            case 2, S.area.interpassMode = 'only';
            case 3, S.area.interpassMode = 'all';
        end
        if S.area.adaptiveEnabled
            S.area.hazRatio = readPositiveScalar(S.hUI.efAdaptiveRatio, S.area.hazRatio, 'HAZ Ratio', false);
            if S.area.hazRatio >= 1.0, S.area.hazRatio = 0.55; end
            set(S.hUI.efAdaptiveRatio, 'String', num2str(S.area.hazRatio));
            % Only clear cache if adaptive was JUST toggled on (not on every param change)
            if isempty(S.area.boundaryTemps)
                % was already empty — nothing to do
            else
                % Keep existing boundaries; Max Area / HAZ Ratio changes
                % are applied live in getMeltThresholds without re-calculation.
            end
            % Grey out fixed-threshold inputs
            set(S.hUI.efAreaMP, 'Enable', 'off', 'TooltipString', 'Disabled: using gradient boundary detection');
            set(S.hUI.efAreaHAZ, 'Enable', 'off', 'TooltipString', 'Disabled: using gradient boundary detection');
        else
            set(S.hUI.efAreaMP, 'Enable', 'on', 'TooltipString', '');
            set(S.hUI.efAreaHAZ, 'Enable', 'on', 'TooltipString', '');
        end
        updateFrame(S.frameIdx);
    end
    ...
    function bTemp = detectMeltPoolBoundaryTemp(img, refTemp, minRadials)
        ...
        % Reject if the hottest pixel is far below the expected phase-change
        % temperature. Even accounting for emissivity effects, a frame that
        % does not reach ~92 % of refTemp cannot contain a liquid phase.
        if maxVal < refTemp * 0.92
            bTemp = NaN; return;
        end
        ...
        boundTemps = []; boundDists = []; boundGrads = [];
        for theta = angles
            nPts = min(60, maxR);
            ...
            grad = abs(diff(prof));
            [gradMax, gIdx] = max(grad);
            if gIdx > 1 && gIdx < length(prof)
                boundTemps(end+1) = prof(gIdx);
                boundDists(end+1) = r(gIdx);
                boundGrads(end+1) = gradMax;
            end
        end
        ...
        if isempty(boundTemps)
            ...
            % [v8.83] False-positive rejection pipeline (5 guards).
            %
            % Guard 1: Minimum thermal contrast. Uniform frames (beam off,
            % cool-down) have no hot spot and must not trigger detection.
            minBg = min(img(:));
            if (maxVal - minBg) < 50
                bTemp = NaN; return;
            end

            % Guard 2: Boundary must be close to beam centre. A real melt
            % pool is compact; a hot part edge is far from the nominal centre.
            MAX_BOUNDARY_DIST = min(H, W) / 4;
            medDist = median(boundDists);
            if medDist > MAX_BOUNDARY_DIST
                bTemp = NaN; return;
            end

            % Guard 3: Gradient must be steep. The solid-liquid interface
            % produces a sharp gradient; a cooling part surface does not.
            MIN_GRADIENT = 15;
            medGrad = median(boundGrads);
            if medGrad < MIN_GRADIENT
                bTemp = NaN; return;
            end

            % Guard 4: Boundary temperature must be close to the peak.
            % In a real melt pool the boundary is 70-95 % of T_max;
            % on a cooling part the "boundary" (part edge) is much colder.
            medT = median(boundTemps);
            if medT / maxVal < 0.70
                bTemp = NaN; return;
            end

            % Guard 4b: Detected boundary must be near the expected phase-change
            % temperature (refTemp). If the boundary is far from refTemp, the
            % gradient peak is not a solid-liquid interface but an arbitrary
            % thermal edge on a hot solid part.
            if abs(medT - refTemp) / refTemp > 0.20
                bTemp = NaN; return;
            end

            % Guard 5: Minimum number of valid radial directions.
    ...
    function onGenerateNDTReport(~, ~)
        ...
        xlabel(ax2, 'Distance from Heat Center (px^2)'); ylabel(ax2, 'Temperature (\circC)'); title(ax2, 'Core-Centered Transverse Profile'); legend(ax2, 'Location', 'best'); grid(ax2, 'on');
